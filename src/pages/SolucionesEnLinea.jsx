...
export default function SolucionesEnLinea() {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      setStatusResult({ success: false, message: err.response?.data?.detail || 'No se encontró solicitud' });
    } finally { setIsSearching(false); }
  };

  const statusBadge = { SOLICITADO: 'bg-[#E1A031]/15 text-[#7a5200]', EN_PROCESO: 'bg-[#7CC6D8]/20 text-[#242B57]', COMPLETADO: 'bg-green-100 text-green-800', RECHAZADO: 'bg-red-100 text-red-800' };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-[#7CC6D8]/10 to-[#4996C6]/10">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="mb-10">
          ...
          <h1 className="text-3xl font-bold text-[#242B57]">Soluciones en Línea</h1>
          <p className="text-slate-600 mt-2">Realiza trámites y solicitudes administrativas en línea</p>
        </div>
        ...
        {activeTab === 'nueva' && (
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20 p-8">
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">Tipo de Trámite</label>
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">Descripción de la solicitud</label>
              ...
              <button type="submit" disabled={isSubmitting}
                className="w-full bg-[#242B57] hover:bg-[#4996C6] disabled:bg-gray-400 text-white px-6 py-3 rounded-xl font-semibold shadow-lg transition-all">
